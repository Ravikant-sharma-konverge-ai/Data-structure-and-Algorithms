...
def count_helper(L,R,lst):
    M = (L+R)//2

    if lst[M+1] < lst[M]:
        return M+1
    elif lst[0] > lst[M]:
        return count_helper(L,M,lst)
    elif lst[0] < lst[M]:
        return count_helper(M+1,R,lst)   
...
def count_rotations(nums):
    ...
    if len(nums)<2:
        ...
        if nums[0]<nums[-1]:
            ...
            return count_helper(L,R,nums)

if __name__ == '__main__':

    chk = [10000000,100000001] + list(range(10000000))
    

    print(count_rotations(chk))
